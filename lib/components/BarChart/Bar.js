  ...
  function BarChart() {
    ...
    return _ret = (_temp = (_this = _possibleConstructorReturn(this, (_ref = BarChart.__proto__ || Object.getPrototypeOf(BarChart)).call.apply(_ref, [this].concat(args))), _this), _this.determineScales = function (_ref2) {
      ...
      if (orientation === 'horizontal') {
        var xScale = scalar().domain([0, Math.max.apply(Math, _toConsumableArray(yPoints))]).range([margin.left, width]);
        ...
        var _yScale = scalar().domain([Math.max.apply(Math, _toConsumableArray(yPoints)), 0]).range([height, margin.top]);
  ...
  _createClass(BarChart, [{
    ...
    value: function render() {
      ...
      if (data.map(function (item) {
        ...
        process.env.NODE_ENV !== 'production' && console.warn('BarChart: No data found with dataKey ' + dataKey);
        return null;
      }

      var _determineScales = this.determineScales({ type: type, orientation: orientation }),
      ...
      var barHeight = function barHeight(d) {
        return yScale((0, _get2.default)(d, dataKey));
      };
...
BarChart.propTypes = {
   ...
   */
  inverted: _propTypes2.default.boolean,
  /**

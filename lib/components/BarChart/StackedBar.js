...
var StackedBar = function (_Component) {
  ...
  function StackedBar() {
    ...
    return _ret = (_temp = (_this = _possibleConstructorReturn(this, (_ref = StackedBar.__proto__ || Object.getPrototypeOf(StackedBar)).call.apply(_ref, [this].concat(args))), _this), _this.determineScales = function (_ref2) {
      ...
      var dataDomain = Math.max.apply(Math, _toConsumableArray((0, _flatten2.default)(data.map(function (d) {
        return keys.map(function (key) {
          return (0, _get2.default)(d, key);
        });
      }).map(function (arr) {
        return (0, _sum2.default)(arr);
      }))));

      var scalar = type === 'linear' ? _d3Scale.scaleLinear : _d3Scale.scaleTime;

      if (orientation === 'horizontal') {
        var xScale = scalar().domain([0, dataDomain]).range([margin.left, width]);
        ...
        var _yScale = scalar().domain([dataDomain, 0]).range([height, margin.top]);
  ...
  _createClass(StackedBar, [{
    ...
    value: function render() {
      ...
      var _determineScales = this.determineScales({ type: type, orientation: orientation, data: data, keys: keys }),
      ...
      var yPoint = function yPoint(d) {
        return yScale((0, _get2.default)(d, yKey));
      };
      var xPoint = function xPoint(d) {
        return xScale((0, _dataUtils.extractX)(d, xKey));
      };
        ...
        series && series.map(function (s, i) {
          return _react2.default.createElement(
            ...
            s.map(function (d, ii) {
              ...
              return _react2.default.createElement(_styledComponents.StyledBar, _extends({
                key: 'bar-group-bar-' + i + '-' + ii + '-' + s.key,
                x: isHorizontal ? xScale(d[0]) : xPoint((0, _get2.default)(d, 'data')),
                y: isHorizontal ? yPoint((0, _get2.default)(d, 'data')) : height + margin.top - yScale(d[1]),
                ...
                onMouseMove: function onMouseMove() {
                  return function (event) {
                    var key = s.key;
                    var datum = (0, _set2.default)({}, xKey || 'xValue', (0, _head2.default)((0, _dataUtils.extractX)((0, _get2.default)(d, 'data'), xKey)));
                    (0, _set2.default)(datum, key, (0, _get2.default)(d, 'data.' + key));
                    return notool || mouseMove({ event: event, datum: datum });
                  };
                },

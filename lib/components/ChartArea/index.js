  ...
  function ChartArea() {
    ...
    return _ret = (_temp = (_this = _possibleConstructorReturn(this, (_ref = ChartArea.__proto__ || Object.getPrototypeOf(ChartArea)).call.apply(_ref, [this].concat(args))), _this), _this.state = {
      ...
      flow(function (xValue) {
        ...
        return updateTooltip({ calculatedData: calculatedData, x: x, yCoords: yCoords });
      })(xValue);
  ...
  _createClass(ChartArea, [{
    ...
    value: function render() {
      ...
      return chartData && React.createElement(
        ...
        React.createElement(
          'svg',
          { width: parentWidth, height: parentHeight, preserveAspectRatio: 'none', viewBox: viewBox ? viewBox : determineViewBox({ biaxialChildren: biaxialChildren, margin: margin, parentWidth: parentWidth, parentHeight: parentHeight }), ref: function ref(svg) {
          ...
          React.createElement(
            Group,
            { left: margin.left },
            React.createElement(Bar, { width: width - margin.left, height: height, fill: 'transparent', onMouseMove: function onMouseMove() {
                return function (event) {
                  _this2.mouseMove({ event: event });
                };
              }, onMouseLeave: function onMouseLeave() {
                return _this2.mouseLeave;
              } }),
            React.createElement(StyledGridRows, _extends({ scale: yScale }, { stroke: stroke }, { width: width - margin.left })),
            biaxialChildren || React.createElement(StyledLeftAxis, { scale: yScale, color: color, hideTicks: true, tickFormat: formatY })
          ),
          React.createElement(StyledBottomAxis, _extends({ scale: xScale }, { color: color, height: height }, { hideTicks: true, tickFormat: formatX })),
          x && React.createElement(Indicator, { yCoords: yCoords, x: x, stroke: stroke, color: color })
        ),
        x && React.createElement(Tooltip, { tooltipData: calculatedData, x: x, color: color })
      );
    }
  }]);
